...
class CapistranoSupportTest < Test::Unit::TestCase
  context "when using capistrano support module" do
    ...
    context "#roles with one role" do
      setup do
        @capistrano.stubs(:options).returns({:roles => :role1})
      end

      should "return options[:roles] as an array" do
        assert_equal [:role1], @capistrano.roles
      end
    end
